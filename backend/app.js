...
let agentInformation = {
  "James": {
    ...
    Your three teammates are Sophia, Ethan, and a participant who will give you their name. Feel free to call people out by name and ask questions. Please call the other participant out by their name. Their messages are most important.
    - East Point Mall: 
...
async function decideParticipation(conversationHistory, agentName) {
  ...
  It's good to respond early on in the conversation, but it's also good to take breaks. You should generally not respond, and "pick your spots" only when it's important.
    ...
    messages: [
      {
        ...
        content: `You are an AI impersonating a human, who is on a new team of purchasing executives for Big Restaurant in a chat-based discussion. 
                  ...
                  - ALWAYS USE THE PREVIOUS CHAT AND TREAT IT AS AN ONGOING CHAT. CALL OUT OR ASK OTHER TEAMATES QUESTIONS.
...
app.post('/ask-openai', async (req, res) => {
  try {
    ...
    // Add the user's message to the conversation history with star emojis for the transcript
    if (message) {
      conversationHistory.push({
        role: 'user',
        content: `⭐${message}⭐` // Append star emojis to the participant's message
      });
    }
